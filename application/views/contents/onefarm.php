...
<div class="modal fade" id="maintProg" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	...
	<script async defer type="text/javascript">
		...
		$('.savecateg').on('click',function(){
			...
			$.ajax({
					url: "http://"+window.location.host+"/shl/main/updateFarmType",
					type: "POST",
					data: {"data":send},
					success: function(data){ 
						// alert(data);
						$("#reloadDiv").load("http://"+window.location.host+"/shl/main/getOneFarm",{id:id});
		...
		$('#Saveedituser').on('click',function(){
			...
			if (password==cpassword) {
				console.log('oki');
				$.ajax({
					url: "http://"+window.location.host+"/shl/main/updateUser",
					type: "POST",
					data: {"data":userdet},
					success: function(data){
  ...
  $(".modal-btn-is").on("click", function(){
	  ...
	  $('.'+splitString+'im-modal').modal('hide');
		// alert(delact);
    $.ajax({
		url: "http://"+window.location.host+"/shl/main/deleteImage",
		type: "POST",
		data: {"data":delact},
		success: function(data){
			// alert(data[0]+" "+data[1]);
			$("#reloadDiv").load("http://"+window.location.host+"/shl/main/getOneFarm",{id:data});
			var fade_in = function() {
				  // $(".alert").fadeOut().empty();
 ...
 // delete accom 
  ...
  $(".modal-btn-oo").on("click", function(){
    ...
    $.ajax({
		url: "http://"+window.location.host+"/shl/main/deleteAccom",
		type: "POST",
		data: {"data":delaccom},
		success: function(data){
 ...
 // delete act 
  ...
  $(".modal-btn-si").on("click", function(){
      ...
      var delact = new Array(splitString,farm_id);
		...
		// alert(delact);
    $.ajax({
		url: "http://"+window.location.host+"/shl/main/deleteActivity",
		type: "POST",
		data: {"data":delact},
		success: function(data){
			// alert(data[0]+" "+data[1]);
  ...
  $(".modal-btn-no").on("click", function(){
    ...
    $('.'+splitString+'mi-modal').modal('hide');
		...
		$(document).on("click",".editAccom", function(event) {
	  	...
	  	$('.editAccomDone').on('click',function(event){
			...
			$.ajax({
				url: "http://"+window.location.host+"/shl/main/updateAccommodation",
				type: "POST",
		...
		$(document).on("click",".editActivities", function(event) {
	  	...
	  	$('.editDone').on('click',function(){
			...
			$.ajax({
				url: "http://"+window.location.host+"/shl/main/updateActivity",
				type: "POST",
	    ...
	    $('.saveeditfarm').on('click',function(event){
	    	...
	    	var frmdetails = new Array(frm_info,frm_info2);
	 		...
	 		$.ajax({
				url: "http://"+window.location.host+"/shl/main/updateFarm",
				type: "POST",
	...
	$( "#featSave" ).click(function() {
		...
		$cropp.croppie('result', {
			...
			$.ajax({
				url: "http://"+window.location.host+"/shl/upload.php",
				type: "POST",
	...
	function saveToDbFeat(data,id,imgdesc){
		...
		$.ajax({
		    type: 'GET',
		    url: 'http://'+window.location.host+'/shl/main/addFeatPic?link='+data+'&id='+id+'&desc='+imgdesc,
		    success: function(data) {
		    	// alert(data);
		    	$("#reloadDiv").load("http://"+window.location.host+"/shl/main/getOneFarm",{id:data});
		    	var fade_in = function() {
...
function saveToDb(data,id){
	$.ajax({
	    ...
	    type: 'GET',
	    url: 'http://'+window.location.host+'/shl/main/addLogoPic?link='+data+'&id='+id,
